...
vi ans;
void dfs(int src, int parent, vvi& g){
    ans.push_back(src);
    for (const auto &item: g[src]){
        if(item != parent){
            dfs(item,src,g);
            ans.push_back(src);
        }
...
void dfs1(int node, int depth, vector<int>& depths, int parent , vvi& g) {
    depths[node] = depth;
    for (int neighbor : g[node]) {
        if (neighbor != parent) {
            dfs1(neighbor, depth + 1, depths, node,g);
        }
    }
}

pair<int, int> minPair(const pair<int, int>& a, const pair<int, int>& b) {
...
class SegmentTree {
    ...
    void buildTree(const vector<pair<int, int>>& arr, int start, int end, int node) {
        if (start == end) {
            tree[node] = arr[start];
        } else {
            int mid = (start + end) / 2;
            ...
            tree[node] = minPair(tree[2 * node + 1], tree[2 * node + 2]);
        }
    }
    
    pair<int, int> queryUtil(int node, int start, int end, int l, int r) {
        if (r < start || l > end) {
            return {INT_MAX, -1}; // Return a large value that won't affect the minimum
        }
        if (l <= start && r >= end) {
            return tree[node];
        }
        int mid = (start + end) / 2;
        pair<int, int> left = queryUtil(2 * node + 1, start, mid, l, r);
        pair<int, int> right = queryUtil(2 * node + 2, mid + 1, end, l, r);
...
void solve(){
    ...
    dfs1(1, 0, depths,-1,graph);
    for (int i = 0; i < ans.size(); ++i) {
        proty.push_back({depths[ans[i]],ans[i]});
    }
    
//    debug(proty);

    SegmentTree st(proty);
    cout<<st.query(4,6).second;
